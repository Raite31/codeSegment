...
export default {
	...
	languageConfirm(e) {
		...
		if (typeof lang != 'undefined') {
			uni.setStorageSync('custom_sys_language', lang); //自定义语言
			uni.setStorageSync('sys_language', lang); //系统语言
			setTimeout(() => {
    ...
    uni.getSystemInfo({
        success: function (res) {
            let lang = uni.getStorageSync('custom_sys_language');
            if(!lang){
                //未指定则通过设备信息获取
	...
	traditionalized(cc) {
		...
		for (var i = 0; i < cc.length; i++) {
			if (this.charPYStr().indexOf(cc.charAt(i)) != -1) {
				console.log('进入判断1 繁化');
				str += this.ftPYStr().charAt(this.charPYStr().indexOf(cc.charAt(i)));
			} else {
				console.log('进入判断2 简化');
				str += cc.charAt(i);
			}
		}
		console.log('Str: ', str);
	...
	simplized(cc) {
		...
		for (var i = 0; i < cc.length; i++) {
			if (this.ftPYStr().indexOf(cc.charAt(i)) != -1)
				str += this.charPYStr().charAt(this.ftPYStr().indexOf(cc.charAt(i)));
	...
	langReplace(str) {
		...
		if (sysLang == 'zh-TW') {
			return this.traditionalized(str);
		}
		if (LangPackArr == '') {
